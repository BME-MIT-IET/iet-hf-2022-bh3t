...
namespace RDFSharpSpecflowTests.StepDefinitions
{
    ...
    public class QueriesTestingStepDefinitions
    {
        ...
        public void GivenAGraphWithPhonesAndTheirBatteryCapacity()
        {
            graph = new RDFGraph();
        ...
        public void ThenReceivingTheActualPhonesWithTheLongestBatteryLife(int tops)
        {
            ...
            {
                Assert.AreEqual(6000 - (i * 500), int.Parse(result.SelectResults.Rows[i].ItemArray[1].ToString().Split("^")[0]));
            }
        }
        ...
        public void ThenTheAverageOfTemperaturesShouldBeCalculatedCorrectlyForEachDay(int numOfDays)
        {
            for (int i = 0; i < numOfDays; i++)
            {
                Assert.AreEqual(24 + i * 2, int.Parse(result.SelectResults.Rows[i].ItemArray[1].ToString().Split("^")[0]));
            }
        }
    }
}
